...
function draw(t){ctx.clearRect(0,0,width,height);const sec=t/1000;const rgb=hexRgb(state.color); for(const p of particles){const reveal=1;let breath=(1-Math.cos(sec*state.speed/42*Math.PI*2/p.period+p.phase))/2;let alpha=(.18+.46*(p.x/Math.max(width,1)/1.1))*(.88+(breath-.5)*.7*p.depth*(state.speed/100))*reveal;if(state.pointer&&pointer.active){const dx=p.x-pointer.x,dy=p.y-pointer.y,dist=Math.hypot(dx,dy);if(dist<180){const force=(1-dist/180)**2*.9;p.x+=(dx/dist||0)*force;p.y+=(dy/dist||0)*force}}const s=state.size*(.42+(p.scatter/.78)*.2)*(0.9+breath*.12);if(state.glow>0){ctx.shadowBlur=state.glow/5;ctx.shadowColor=state.color}ctx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.03,Math.min(.9,alpha))})`;ctx.fillRect(p.x-s/2,p.y-s/2,s,s)}ctx.shadowBlur=0;raf=requestAnimationFrame(draw)}
