...
function resize(){const r=canvas.getBoundingClientRect(); dpr=Math.min(2,window.devicePixelRatio||1); width=r.width;height=r.height;canvas.width=Math.floor(width*dpr);canvas.height=Math.floor(height*dpr);ctx.setTransform(dpr,0,0,dpr,0,0); $('canvasSize').textContent=`${Math.round(width)} × ${Math.round(height)}`; build()}
function build(){particles=[]; const gap=state.lowPerf?11:9; const cols=Math.ceil(width/gap)+1, rows=Math.ceil(height/gap)+1; for(let y=0;y<rows;y++) for(let x=0;x<cols;x++){const edge=cols*(.04+hash(state.seed,401,Math.floor(y/5))*.24);const edgeDensity=Math.min(1,Math.max(.08,(x-edge+2)/4));const scatter=hash(state.seed,x,y);if(scatter>(.22+state.density/100*.78)*edgeDensity)continue;particles.push({x:x*gap+4,y:y*gap+4,scatter,phase:hash(state.seed,x+131,y+211)*Math.PI*2,period:3.8+hash(state.seed,x+47,y+73)*4.4,depth:.65+hash(state.seed,x+307,y+419)*.35,seedX:x,seedY:y})} $('particleCount').textContent=`${particles.length.toString().padStart(2,'0')},${String(particles.length*2).padStart(3,'0')} PARTICLES`}
function draw(t){ctx.clearRect(0,0,width,height);const sec=t/1000;const rgb=hexRgb(state.color); for(const p of particles){const reveal=1;let breath=(1-Math.cos(sec*state.speed/42*Math.PI*2/p.period+p.phase))/2;let alpha=(.18+.46*(p.x/Math.max(width,1)/1.1))*(.88+(breath-.5)*.7*p.depth*(state.speed/100))*reveal;if(state.pointer&&pointer.active){const dx=p.x-pointer.x,dy=p.y-pointer.y,dist=Math.hypot(dx,dy);if(dist<180){const force=(1-dist/180)**2*.9;p.x+=(dx/dist||0)*force;p.y+=(dy/dist||0)*force}}const s=state.size*(.42+(p.scatter/.78)*.2)*(0.9+breath*.12);if(state.glow>0){ctx.shadowBlur=state.glow/5;ctx.shadowColor=state.color}ctx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.03,Math.min(.9,alpha))})`;ctx.beginPath();ctx.roundRect(p.x-s/2,p.y-s/2,s,s,1.5);ctx.fill()}ctx.shadowBlur=0;raf=requestAnimationFrame(draw)}
...
function mountHeroDotfield(el) {
  ...
  let w=0,h=0,scale=1,items=[],mouse={x:-9999,y:-9999,active:false};
  const resizeHero=()=>{const r=el.getBoundingClientRect(); scale=Math.min(2,devicePixelRatio||1);w=r.width;h=r.height;el.width=w*scale;el.height=h*scale;cx.setTransform(scale,0,0,scale,0,0);const gap=Math.max(7,Math.min(10,w/72));const rows=bitmap.length;const cols=Math.max(...bitmap.map(line=>line.length));const startX=(w-cols*gap)/2;const startY=(h-rows*gap)/2;items=[];for(let y=0;y<rows;y++){for(let x=0;x<cols;x++){const char=bitmap[y][x]||' ';const lit=char==='1';items.push({x:startX+x*gap,y:startY+y*gap,lit,phase:hash(23,x,y)*Math.PI*2,seed:hash(23,x+11,y+7)});}}};
  const paintHero=(time)=>{cx.clearRect(0,0,w,h);const sec=time/1000;for(const p of items){let dx=0,dy=0;const dist=Math.hypot(p.x-mouse.x,p.y-mouse.y);if(mouse.active&&dist<95){const f=(1-dist/95)**2*9;dx=(p.x-mouse.x)/(dist||1)*f;dy=(p.y-mouse.y)/(dist||1)*f}const breath=(1-Math.cos(sec*(state.speed/42||1)*1.5+p.phase))/2;const s=p.lit?5.7+breath*1.8:4.8;cx.fillStyle=p.lit?`rgba(172,137,216,${.62+breath*.24})`:`rgba(196,183,220,${.1+p.seed*.12})`;cx.beginPath();cx.roundRect(p.x+dx-s/2,p.y+dy-s/2,s,s,1.5);cx.fill()}requestAnimationFrame(paintHero)};
