...
function apply(){['density','speed','size','height','glow'].forEach(id=>$(id).value=state[id]);$('densityValue').textContent=state.density+'%';$('speedValue').textContent=state.speed+'%';$('sizeValue').textContent=state.size+' px';$('heightValue').textContent=state.height+' px';$('glowValue').textContent=state.glow+'%';$('particleColor').value=state.color;$('colorValue').textContent=state.color.toUpperCase();canvas.parentElement.style.background=presets[state.preset]?.bg||'#131516';canvas.parentElement.style.height=state.height+'px';document.querySelectorAll('.preset').forEach(b=>b.classList.toggle('active',b.dataset.preset===state.preset));resize();updateCode()}
...
function mountDriftfield(canvas, options = {}) {
  const ctx = canvas.getContext('2d');
  const config = { color: '${state.color}', height: ${state.height}, seed: 23, density: ${state.density / 100}, speed: ${state.speed / 42}, size: ${state.size / 3}, height: ${state.height}, glow: ${state.glow}, pointer: ${state.pointer}, lowPerf: ${state.lowPerf}, gap: 9, ...options };
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      ctx.fillRect(left + x * config.gap + 4 - size / 2, y * config.gap + 4 - size / 2, size, size);
    }
