...
function mountTabTexture(el, seed) {
  ...
  const resizeTab=()=>{const r=el.getBoundingClientRect();w=r.width;h=r.height;dpr=Math.min(1.5,devicePixelRatio||1);el.width=w*dpr;el.height=h*dpr;cx.setTransform(dpr,0,0,dpr,0,0);const gap=9;cols=Math.ceil(w/gap)+1;rows=Math.ceil(h/gap)+1;items=[];for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){const edge=cols*(.04+hash(seed,401,Math.floor(y/5))*.24);const edgeDensity=Math.min(1,Math.max(.08,(x-edge+2)/4));const scatter=hash(seed,x,y);if(scatter>.78*edgeDensity)continue;const sample=createParticleSample(seed,x,y,cols);if(sample)items.push({x:x*gap+4,y:y*gap+4,sample})}};
  const paintTab=(time)=>{if(time-last<33){requestAnimationFrame(paintTab);return}last=time;cx.clearRect(0,0,w,h);const rgb=hexRgb(state.color);const sec=time/1000;for(const p of items){const appearance=p.sample(sec,1,state.speed/42);if(!appearance)continue;let ox=0,oy=0,boost=0;const dist=Math.hypot(p.x-pointer.x,p.y-pointer.y);if(state.pointer&&pointer.active&&dist<100){const force=(1-dist/100)**2*8;ox=(p.x-pointer.x)/(dist||1)*force;oy=(p.y-pointer.y)/(dist||1)*force;boost=(1-dist/100)**2*.34}const alpha=Math.min(.96,appearance.strength/100+boost);const size=8*appearance.size+boost*3;cx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.03,alpha)})`;cx.fillRect(p.x+ox-size/2,p.y+oy-size/2,size,size)}requestAnimationFrame(paintTab)};
