...
function mountTabTexture(el,seed){
  ...
  const paintTab=(time)=>{if(time-last<33){requestAnimationFrame(paintTab);return}last=time;cx.clearRect(0,0,w,h);const rgb=hexRgb(state.color);const sec=time/1000;const amplitude=Math.max(.2,state.speed/42);const phase=sec*1.5;const left=w-(cols*9-1);for(const p of items){const edgeNoise=flowingNoise(seed,0,p.y+phase*.45);const edge=cols*(.385+(edgeNoise-.5)*.33*amplitude);const distance=p.x-edge;if(distance<-3)continue;const noise=flowingNoise(seed,p.x+11,p.y+7+phase);const ember=distance<0&&distance>=-3&&noise>.78;const visible=distance>=0?distance>2||noise>.18:ember;if(!visible)continue;const coverage=ember?Math.min(1,(noise-.78)/.15):Math.min(1,Math.max(0,distance+.5));const tailFade=ember?1:Math.min(1,.2+Math.max(0,distance)/18*.8);let alpha=(ember?.24+(noise-.78)*.35:(.12+(p.x/Math.max(1,cols-1))*.24+noise*.12)*tailFade)*coverage;let ox=0,oy=0;const px=left+p.x*9+4,py=p.y*9+4;const dist=Math.hypot(px-pointer.x,py-pointer.y);if(state.pointer&&pointer.active&&dist<100){const force=(1-dist/100)**2*8;ox=(px-pointer.x)/(dist||1)*force;oy=(py-pointer.y)/(dist||1)*force;alpha=Math.min(.96,alpha+(1-dist/100)**2*.34)}const size=ember?3.4+noise*2.4:6.6+Math.min(1,Math.max(0,distance)/18)*1.4;cx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.025,Math.min(.9,alpha))})`;cx.fillRect(px+ox-size/2,py+oy-size/2,size,size)}requestAnimationFrame(paintTab)};
