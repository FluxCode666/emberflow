...
function burningCell(seed, x, y, columns, time, visibility = 1, amplitude = 1) {
  ...
  return { alpha, size: hotEmber ? 3.5 : trail ? 3 : 7, distance };
}
...
function mountDriftfield(canvas, options = {}) {
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const tailDepth = Math.max(0, -cell.distance);
      const curl = (flowingNoise(config.seed + 211, x, y + time / 1000 * 1.4) - 0.5) * 4 + Math.sin(y * 0.45 + time / 1000 * 2.1) * 1.5;
      const px = left + x * config.gap + 4, py = y * config.gap + 4 - tailDepth * 0.22 * config.gap + curl * (tailDepth / 32);
