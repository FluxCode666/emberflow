...
function burningCell(seed, x, y, columns, time, visibility = 1, amplitude = 1, sampleX = x) {
  const fieldY = y * 0.42;
  ...
  const tailLimit = 18 + flowingNoiseX(seed + 211, x * 0.18 + time * 0.18, y * 0.18 + time * 0.14 + 41) * 14;
...
function mountDriftfield(canvas, options = {}) {
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const cell = burningCell(config.seed, x, y, columns, time / 1000, config.density, config.speed, sampleX);
      if (!cell) continue;
