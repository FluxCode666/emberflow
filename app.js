...
function mountDriftfield(canvas, options = {}) {
  const ctx = canvas.getContext('2d');
  const config = { color: '${state.color}', seed: 23, density: ${state.density / 100}, speed: ${state.speed / 42}, size: ${state.size / 3}, height: ${state.height}, glow: ${state.glow}, pointer: ${state.pointer}, lowPerf: ${state.lowPerf}, gap: 9, ...options };
