...
function burningCell(seed, x, y, columns, time, visibility = 1, amplitude = 1, sampleX = x) {
  const edgeNoise = flowingNoise(seed, 0, y);
  const wave = Math.sin(y * 0.72) * 2.4;
  const edge = columns * (0.385 + (edgeNoise - 0.5) * 0.33 * amplitude) + wave;
  const distance = x - edge;
  if (distance < -32) return null;
  const noise = flowingNoiseX(seed, sampleX + 11, y + 7);
  const tailNoise = flowingNoiseX(seed + 97, sampleX + 7, y + 31);
  ...
  const alpha = (hotEmber ? 0.2 + (noise - 0.52) * 0.62 : trail ? 0.06 + tailNoise * 0.16 : 0.12 + sampleX / columns * 0.24 + noise * 0.12) * coverage;
...
function mountDriftfield(canvas, options = {}) {
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const sampleX = (x + flowOffset / config.gap) % columns;
