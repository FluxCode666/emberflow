...
function burningCell(seed, x, y, columns, time, visibility = 1, amplitude = 1, sampleX = x) {
  ...
  const noise = flowingNoise(seed, sampleX + 11, y + 7 + phase * 1.15);
  const tailNoise = flowingNoise(seed + 97, sampleX + 7, y + 31 + phase * 1.8);
...
function mountDriftfield(canvas, options = {}) {
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const sampleX = (x + Math.floor(flowOffset / config.gap)) % columns;
