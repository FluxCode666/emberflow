...
function burningCell(seed, x, y, columns, time, visibility = 1, sampleX = x, tailVariance = 8) {
  const fieldY = y * 0.42;
...
function mountDriftfield(canvas, options = {}) {
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const cell = burningCell(config.seed, x, y, columns, time / 1000, config.density, sampleX, config.tailVariance);
      if (!cell) continue;
