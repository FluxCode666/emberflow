...
function mountDriftfield(canvas, options = {}) {
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const flowFactor = Math.min(1, tailDepth / 12);
      const flameDrift = (time / 1000 * (7 + config.speed * 8) + y * 0.85 + cell.alpha * 17) % 22 * flowFactor;
      const px = left + x * config.gap + 4 - flameDrift, py = y * config.gap + 4;
