...
const state = { density: 68, speed: 42, size: 3, tailVariance: 8, height: 320, glow: 24, pointerStrength: 30, color: '#f5f0e8', pointer: true, lowPerf: false, seed: 23, preset: 'midnight' };
...
let lastFrame=0;
function draw(t){if(t-lastFrame<33){raf=requestAnimationFrame(draw);return}lastFrame=t;ctx.clearRect(0,0,width,height);const sec=t/1000;const rgb=hexRgb(state.color);const amplitude=Math.max(.2,state.speed/42);const cols=Math.ceil(width/9)+1;const gridWidth=cols*9;const flowOffset=sec*(6+amplitude*10);const left=width-(gridWidth-1);for(const p of particles){const fieldY=p.gy*.42;const edgeNoise=flowingNoise(state.seed,0,fieldY);const baseEdge=cols*(.385+(edgeNoise-.5)*.33*amplitude);const visibility=state.density/100;const recession=1-visibility;const wave=Math.sin(p.gy*.34+state.seed*.01)*2.4+(flowingNoise(state.seed+19,0,fieldY)-.5)*10;const edge=baseEdge+recession*(cols+5-baseEdge)+recession*Math.sin(fieldY*.7)*2+wave;const distance=p.gx-edge;if(distance<-32)continue;const sampleX=p.gx+flowOffset/9;const fieldX=sampleX*.42;const noise=flowingNoiseX(state.seed,fieldX+11,fieldY+7);const tailNoise=flowingNoiseX(state.seed+97,fieldX+7,fieldY+31);const tailDrift=flowingNoiseX(state.seed+503,p.gx*.16+sec*.48,p.gy*.16+23);const tailSignal=Math.max(0,Math.min(1,tailNoise+(tailDrift-.5)*.26));const tailVariance=Math.max(3,Math.min(20,state.tailVariance));const tailLimit=32-tailVariance+flowingNoiseX(state.seed+211,p.gx*.18+sec*.18,p.gy*.18+sec*.14+41)*tailVariance;if(distance<-tailLimit)continue;const tone=Math.min(1,Math.max(0,(noise-.16)/.72));const tailTone=Math.min(1,Math.max(0,(tailSignal-.18)/.82));const hotEmber=distance<0&&distance>=-8&&noise>.52;const trail=distance<-8&&tailSignal>.52+(-distance/tailLimit)*.18;const ember=hotEmber||trail;const visible=distance>=0?distance>2||noise>.18:ember;if(!visible)continue;const coverage=hotEmber?Math.min(1,(noise-.52)/.48):trail?Math.max(.2,(distance+tailLimit)/(tailLimit-8)):Math.min(1,Math.max(0,distance+.5));const tailFade=ember?1:Math.min(1,.2+Math.max(0,distance)/18*.8);let alpha=(hotEmber?.28+tone*.62:trail?.025+tailTone*.3:(.07+(p.gx/Math.max(1,cols-1))*.34+tone*.24)*tailFade)*coverage;const px=left+p.gx*9+4,py=p.gy*9+4;let ox=0,oy=0;const dist=Math.hypot(px-pointer.x,py-pointer.y);if(state.pointer&&pointer.active&&dist<180){const repel=state.pointerStrength/30;const force=(1-dist/180)**2*8*repel;ox=(px-pointer.x)/(dist||1)*force;oy=(py-pointer.y)/(dist||1)*force;alpha=Math.min(.96,alpha+(1-dist/180)**2*.34*repel)}const size=(hotEmber?3.4+noise*2.4:trail?2.2+tailSignal*2.2:6.6+Math.min(1,Math.max(0,distance)/18)*1.4)*(state.size/3);ctx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.025,Math.min(.9,alpha))})`;ctx.fillRect(px+ox-size/2,py+oy-size/2,size,size)}raf=requestAnimationFrame(draw)}
function hexRgb(hex){const v=hex.replace('#','');return [parseInt(v.slice(0,2),16),parseInt(v.slice(2,4),16),parseInt(v.slice(4,6),16)]}
function apply(){['density','speed','size','tailVariance','height','glow','pointerStrength'].forEach(id=>$(id).value=state[id]);$('densityValue').textContent=state.density+'%';$('speedValue').textContent=state.speed+'%';$('sizeValue').textContent=state.size+' px';$('tailVarianceValue').textContent=state.tailVariance+' 格';$('heightValue').textContent=state.height+' px';$('glowValue').textContent=state.glow+'%';$('pointerStrengthValue').textContent=state.pointerStrength+'%';$('particleColor').value=state.color;$('colorValue').textContent=state.color.toUpperCase();canvas.parentElement.style.background=presets[state.preset]?.bg||'#131516';canvas.parentElement.style.height=state.height+'px';document.querySelectorAll('.preset').forEach(b=>b.classList.toggle('active',b.dataset.preset===state.preset));resize();updateCode()}
...
function burningCell(seed, x, y, columns, time, visibility = 1, amplitude = 1, sampleX = x, tailVariance = 8) {
  const fieldY = y * 0.42;
  ...
  const tailLimit = 32 - tailVariance + flowingNoiseX(seed + 211, x * 0.18 + time * 0.18, y * 0.18 + time * 0.14 + 41) * tailVariance;
  ...
  const coverage = hotEmber ? Math.min(1, (noise - 0.52) / 0.48) : trail ? Math.max(0.2, (distance + tailLimit) / (tailLimit - 8)) : Math.min(1, Math.max(0, distance + 0.5));
  const alpha = (hotEmber ? 0.28 + tone * 0.62 : trail ? 0.025 + tailTone * 0.3 : (0.07 + x / columns * 0.34 + tone * 0.24) * (0.2 + Math.max(0, distance) / 18 * 0.8)) * coverage;
  return { alpha, size: hotEmber ? 3.5 : trail ? 3 : 7 };
}

function mountDriftfield(canvas, options = {}) {
  const ctx = canvas.getContext('2d');
  const config = { color: '${state.color}', seed: 23, density: ${state.density / 100}, speed: ${state.speed / 42}, size: ${state.size / 3}, tailVariance: ${state.tailVariance}, height: ${state.height}, glow: ${state.glow}, pointerStrength: ${state.pointerStrength}, pointer: ${state.pointer}, lowPerf: ${state.lowPerf}, gap: 9, ...options };
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const cell = burningCell(config.seed, x, y, columns, time / 1000, config.density, config.speed, sampleX, config.tailVariance);
      if (!cell) continue;
...
function mountTabTexture(el,seed,options={}){
  ...
  const paintTab=(time)=>{if(time-last<33){requestAnimationFrame(paintTab);return}last=time;cx.clearRect(0,0,w,h);const rgb=hexRgb(options.color||state.color);const sec=time/1000;const amplitude=Math.max(.2,options.speed??state.speed/42);const tailWidth=options.tail??52;const tailVariance=options.tailVariance??state.tailVariance;const gridWidth=cols*9;const flowOffset=sec*(6+amplitude*10);const left=w-(gridWidth-1);for(const p of items){const fieldY=p.y*.42;const edgeNoise=flowingNoise(seed,0,fieldY);const wave=Math.sin(p.y*.34+seed*.01)*2.4+(flowingNoise(seed+19,0,fieldY)-.5)*8;const edge=cols*(.385+(edgeNoise-.5)*.33*amplitude)+wave;const distance=p.x-edge;if(distance<-tailWidth)continue;const sampleX=p.x+flowOffset/9;const fieldX=sampleX*.42;const noise=flowingNoiseX(seed,fieldX+11,fieldY+7);const tailNoise=flowingNoiseX(seed+97,fieldX+7,fieldY+31);const tailDrift=flowingNoiseX(seed+503,p.x*.16+sec*.48,p.y*.16+23);const tailSignal=Math.max(0,Math.min(1,tailNoise+(tailDrift-.5)*.26));const tailVariance=options.tailVariance??state.tailVariance;const tailLimit=tailWidth-tailVariance+flowingNoiseX(seed+211,p.x*.18+sec*.18,p.y*.18+sec*.14+41)*tailVariance;if(distance<-tailLimit)continue;const tone=Math.min(1,Math.max(0,(noise-.16)/.72));const tailTone=Math.min(1,Math.max(0,(tailSignal-.18)/.82));const hotEmber=distance<0&&distance>=-8&&noise>.48;const trail=distance<-8&&tailSignal>.48+(-distance/tailLimit)*.2;const ember=hotEmber||trail;const visible=distance>=0?distance>2||noise>.14:ember;if(!visible)continue;const coverage=hotEmber?Math.min(1,(noise-.48)/.52):trail?Math.max(.18,(distance+tailLimit)/(tailLimit-8)):Math.min(1,Math.max(0,distance+.5));const tailFade=ember?1:Math.min(1,.28+Math.max(0,distance)/18*.72);let alpha=(hotEmber?.34+tone*.65:trail?.035+tailTone*.34:(.08+(p.x/Math.max(1,cols-1))*.4+tone*.28)*tailFade)*coverage;let ox=0,oy=0;const px=left+p.x*9+4,py=p.y*9+4;const dist=Math.hypot(px-pointer.x,py-pointer.y);if(state.pointer&&pointer.active&&dist<100){const repel=state.pointerStrength/30;const force=(1-dist/100)**2*8*repel;ox=(px-pointer.x)/(dist||1)*force;oy=(py-pointer.y)/(dist||1)*force;alpha=Math.min(.96,alpha+(1-dist/100)**2*.34*repel)}const size=hotEmber?3.8+noise*2.4:trail?2.4+tailSignal*2.8:6.8+Math.min(1,Math.max(0,distance)/18)*1.4;cx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.025,Math.min(.9,alpha))})`;cx.fillRect(px+ox-size/2,py+oy-size/2,size,size)}requestAnimationFrame(paintTab)};
