...
function burningCell(seed, x, y, columns, time, visibility = 1, sampleX = x, tailVariance = 8, gapRate = 30) {
  ...
  const coverage = emptyRate === 0
    ...
    : hotEmber ? Math.min(1, (noise - 0.52) / 0.48) : trail ? Math.max(0.24, (distance + tailLimit) / (tailLimit - 8)) : Math.min(1, Math.max(0, distance + 0.5));
  const alpha = Math.max((hotEmber ? 0.28 + tone * 0.62 : trail ? 0.035 + tailTone * 0.34 : (0.07 + x / columns * 0.34 + tone * 0.24) * (0.2 + Math.max(0, distance) / 18 * 0.8)) * coverage, emptyRate === 0 ? 0.16 * (distance < 0 ? naturalTailCoverage : 1) : 0);
