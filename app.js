...
function resize(){const r=canvas.getBoundingClientRect(); dpr=Math.min(1.5,window.devicePixelRatio||1); width=r.width;height=r.height;canvas.width=Math.floor(width*dpr);canvas.height=Math.floor(height*dpr);ctx.setTransform(dpr,0,0,dpr,0,0); build()}
function build(){particles=[];const gap=9;const cols=Math.ceil(width/gap)+1,rows=Math.ceil(height/gap)+1;const sampleRate=state.lowPerf?Math.min(1,2200/(cols*rows)):1;for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){if(sampleRate<1&&hash(state.seed+99,x,y)>sampleRate)continue;particles.push({x,y,gx:x,gy:y})}}
...
// Texture flows left on a stationary grid; contour evolution has its own clock.
let lastFrame=0;
function draw(t){if(t-lastFrame<33){raf=requestAnimationFrame(draw);return}lastFrame=t;ctx.clearRect(0,0,width,height);const sec=t/1000;const rgb=hexRgb(state.color);const cols=Math.ceil(width/9)+1;const gridWidth=cols*9;const flowOffset=sec*(6+(state.speed/42)*10);const left=width-(gridWidth-1);const contour=sampleFlameRows(state.seed,Math.ceil((height-4)/9),cols,sec,state.density/100,32,state.tailVariance);for(const p of particles){const fieldY=p.gy*.42;const row=Math.min(p.gy,contour.edges.length-1);const distance=p.gx-contour.edges[row];const tailLimit=contour.tailLimit;if(distance<-tailLimit)continue;const sampleX=p.gx+flowOffset/9;const fieldX=sampleX*.42;const noise=flowingNoiseX(state.seed,fieldX+11,fieldY+7);const tailNoise=flowingNoiseX(state.seed+97,fieldX+7,fieldY+31);const tailDrift=flowingNoiseX(state.seed+503,p.gx*.16+sec*.48,p.gy*.16+23);const rowTail=contour.strengths[row];const tailSignal=Math.max(0,Math.min(1,tailNoise+(tailDrift-.5)*.26+(rowTail-.5)*.16));const tailProgress=Math.max(0,Math.min(1,(-distance-8)/Math.max(1,tailLimit-8)));const tone=Math.min(1,Math.max(0,(noise-.16)/.72));const gapRate=Math.max(0,Math.min(100,state.gapRate))/100;const hotEmber=distance<0&&distance>=-8&&noise>.52;const trail=distance<-8&&(gapRate===0||tailSignal>.3+tailProgress*.34);const ember=hotEmber||trail;const visible=distance>=0?(gapRate===0||distance>2||noise>.18):(gapRate===0||ember);if(!visible)continue;const gapNoise=flowingNoiseX(state.seed+607,fieldX*1.35+fieldY*.1,fieldY*1.15+17);if(gapRate>0&&gapNoise<gapRate)continue;const naturalTailCoverage=Math.max(.45,1-tailProgress*.55);const coverage=gapRate===0?(distance<0?naturalTailCoverage:1):hotEmber?1:trail?Math.max(.24,(distance+tailLimit)/(tailLimit-8)):Math.min(1,Math.max(0,distance+.5));const geometricFade=Math.min(1,.2+Math.max(0,distance)/18*.8);let alpha=Math.max((.07+(p.gx/Math.max(1,cols-1))*.34+tone*.24)*geometricFade*coverage,gapRate===0?.16*(distance<0?naturalTailCoverage:1):0);const px=left+p.gx*9+4,py=p.gy*9+4;let ox=0,oy=0;const dist=Math.hypot(px-pointer.x,py-pointer.y);if(state.pointer&&pointer.active&&dist<180){const repel=state.pointerStrength/30;const force=(1-dist/180)**2*8*repel;ox=(px-pointer.x)/(dist||1)*force;oy=(py-pointer.y)/(dist||1)*force;alpha=Math.min(.96,alpha+(1-dist/180)**2*.34*repel)}const size=(hotEmber?3.4+noise*2.4:trail?2.2+tailSignal*2.2:6.6+Math.min(1,Math.max(0,distance)/18)*1.4)*(state.size/3);ctx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.025,Math.min(.9,alpha))})`;ctx.fillRect(px+ox-size/2,py+oy-size/2,size,size)}raf=requestAnimationFrame(draw)}
...
function burningCell(seed, x, y, columns, time, contour, sampleX = x, gapRate = 0) {
  ...
  const distance = x - contour.edges[row];
  ...
  return { alpha, size: hotEmber ? 3.4 + noise * 2.4 : trail ? 2.2 + tailSignal * 2.2 : 6.6 + Math.min(1, Math.max(0, distance) / 18) * 1.4 };
}

function mountDriftfield(canvas, options = {}) {
  const ctx = canvas.getContext('2d');
  const config = { color: '${state.color}', seed: ${state.seed}, density: ${state.density / 100}, speed: ${state.speed / 42}, size: ${state.size / 3}, tailVariance: ${state.tailVariance}, gapRate: ${state.gapRate / 100}, height: ${state.height}, glow: ${state.glow}, pointerStrength: ${state.pointerStrength}, pointer: ${state.pointer}, lowPerf: ${state.lowPerf}, gap: 9, ...options };
  ...
  const paint = (time) => {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < columns; x++) {
      ...
      const cell = burningCell(config.seed, x, y, columns, time / 1000, contour, sampleX, config.gapRate * 100);
      if (!cell) continue;
...
mountDriftfield(document.querySelector('#particle-canvas'));`;codeVariants=buildCodeVariants(c);renderModalCode()}
...
export type DriftfieldOptions = Partial<{
  seed: number; color: string; density: number; speed: number; size: number;
