...
function build(){particles=[];const gap=9;const maxParticles=state.lowPerf?2200:4200;const cols=Math.ceil(width/gap)+1,rows=Math.ceil(height/gap)+1;for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){if(particles.length>=maxParticles)break;particles.push({x,y,gx:x,gy:y})}$('particleCount').textContent=`${particles.length.toString().padStart(2,'0')},${String(particles.length*2).padStart(3,'0')} PARTICLES`}
let lastFrame=0;
function draw(t){if(t-lastFrame<33){raf=requestAnimationFrame(draw);return}lastFrame=t;ctx.clearRect(0,0,width,height);const sec=t/1000;const rgb=hexRgb(state.color);const amplitude=Math.max(.2,state.speed/42);const phase=sec*1.5;const cols=Math.ceil(width/9)+1;const left=width-(cols*9-1);for(const p of particles){const edgeNoise=flowingNoise(state.seed,0,p.gy+phase*.45);const baseEdge=cols*(.385+(edgeNoise-.5)*.33*amplitude);const visibility=state.density/100;const recession=1-visibility;const edge=baseEdge+recession*(cols+5-baseEdge)+recession*Math.sin(p.gy*.7+phase)*2;const distance=p.gx-edge;if(distance<-8)continue;const noise=flowingNoise(state.seed,p.gx+11,p.gy+7+phase);const ember=distance<0&&distance>=-8&&noise>.52;const visible=distance>=0?distance>2||noise>.18:ember;if(!visible)continue;const coverage=ember?Math.min(1,(noise-.52)/.48):Math.min(1,Math.max(0,distance+.5));const tailFade=ember?1:Math.min(1,.2+Math.max(0,distance)/18*.8);let alpha=(ember?.2+(noise-.52)*.62:(.12+(p.gx/Math.max(1,cols-1))*.24+noise*.12)*tailFade)*coverage;const px=left+p.gx*9+4,py=p.gy*9+4;let ox=0,oy=0;const dist=Math.hypot(px-pointer.x,py-pointer.y);if(state.pointer&&pointer.active&&dist<180){const force=(1-dist/180)**2*8;ox=(px-pointer.x)/(dist||1)*force;oy=(py-pointer.y)/(dist||1)*force;alpha=Math.min(.96,alpha+(1-dist/180)**2*.34)}const size=(ember?3.4+noise*2.4:6.6+Math.min(1,Math.max(0,distance)/18)*1.4)*(state.size/3);ctx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.025,Math.min(.9,alpha))})`;ctx.fillRect(px+ox-size/2,py+oy-size/2,size,size)}raf=requestAnimationFrame(draw)}
...
function mountTabTexture(el,seed){
  ...
  const paintTab=(time)=>{if(time-last<33){requestAnimationFrame(paintTab);return}last=time;cx.clearRect(0,0,w,h);const rgb=hexRgb(state.color);const sec=time/1000;const amplitude=Math.max(.2,state.speed/42);const phase=sec*(1.1+amplitude*.9);const left=w-(cols*9-1);for(const p of items){const edgeNoise=flowingNoise(seed,0,p.y+phase*.9);const wave=Math.sin(p.y*.72+phase*2.1+seed*.01)*2.4+(flowingNoise(seed+19,0,p.y+phase*1.25)-.5)*8;const edge=cols*(.385+(edgeNoise-.5)*.33*amplitude)+wave;const distance=p.x-edge;if(distance<-8)continue;const noise=flowingNoise(seed,p.x+11,p.y+7+phase*1.15);const ember=distance<0&&distance>=-8&&noise>.52;const visible=distance>=0?distance>2||noise>.18:ember;if(!visible)continue;const coverage=ember?Math.min(1,(noise-.52)/.48):Math.min(1,Math.max(0,distance+.5));const tailFade=ember?1:Math.min(1,.2+Math.max(0,distance)/18*.8);let alpha=(ember?.2+(noise-.52)*.62:(.12+(p.x/Math.max(1,cols-1))*.24+noise*.12)*tailFade)*coverage;let ox=0,oy=0;const px=left+p.x*9+4,py=p.y*9+4;const dist=Math.hypot(px-pointer.x,py-pointer.y);if(state.pointer&&pointer.active&&dist<100){const force=(1-dist/100)**2*8;ox=(px-pointer.x)/(dist||1)*force;oy=(py-pointer.y)/(dist||1)*force;alpha=Math.min(.96,alpha+(1-dist/100)**2*.34)}const size=ember?3.4+noise*2.4:6.6+Math.min(1,Math.max(0,distance)/18)*1.4;cx.fillStyle=`rgba(${rgb.join(',')},${Math.max(.025,Math.min(.9,alpha))})`;cx.fillRect(px+ox-size/2,py+oy-size/2,size,size)}requestAnimationFrame(paintTab)};
